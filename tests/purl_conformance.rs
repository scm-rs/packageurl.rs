...
//!
//! The suite is the `package-url/purl-spec` git submodule at `tests/purl-spec`,
//! pinned to a specific commit. Fetch it with
//! `git submodule update --init tests/purl-spec`; the test skips when it is not
//! checked out (unless `PURL_CONFORMANCE_REQUIRED` is set, as it is in CI). Every
//! case is run through the public API and reconciled against `KNOWN_GAPS`, the
//! list of cases the crate does not yet satisfy.
//!
...
//!
//! To bump the suite, check out a new commit in the submodule, then re-baseline:
...
// ---------------------------------------------------------------------------
// Vendored test-suite model (schema: tests/purl-spec/schemas/purl-test.schema-0.1.json)
// ---------------------------------------------------------------------------
...
/// Loads every case, or `None` when the `tests/purl-spec` submodule is absent.
fn load() -> Option<Vec<(String, Case)>> {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/purl-spec/tests");
...
fn purl_spec_conformance() {
    let cases = match load() {
        Some(cases) => cases,
        None => {
            let hint = "purl-spec submodule not checked out — run `git submodule update --init tests/purl-spec`";
            assert!(
                std::env::var_os("PURL_CONFORMANCE_REQUIRED").is_none(),
                "{hint}"
            );
            eprintln!("skipping purl-spec conformance: {hint}");
            return;
        }
    };
    assert!(
        !cases.is_empty(),
        "purl-spec submodule is present but no cases were found"
    );
